...
The simulator models PV generation, wind turbines, batteries, DC/DC converters, household loads, and grid-interconnection behavior using simplified equivalent-circuit models.

This repository provides:
- Residential microgrid models (`Residential_grid8.slx`, `Residential_grid9.slx`)
- Legacy models (Residential_grid1.slx – Residential_grid7.slx)
  *All previous versions are preserved for reference and reproducibility.*
...
- **Enhanced curtailment based on DC baseline over-voltage (Ver.2.2)**  
  → Significantly reduces DC bus voltage rise under high renewable generation

### Wind turbine model
- Cut-in / rated / cut-out wind-speed characteristics
- Output-limiting function based on bus voltage
- Over-charging output suppression
- **Improved over-voltage curtailment logic (Ver.2.2)**
...
### Legacy Models
- `Residential_grid5.slx` (Ver.2.1)
- `Residential_grid7.slx` (Ver.2.1)
...
https://doi.org/10.5281/zenodo.21720183
...
1. Open the desired Simulink model (`Residential_grid8.slx` or `Residential_grid9.slx`)
2. Import all required `.mat` files into the MATLAB workspace
3. Set simulation end time (e.g., one year = `365*24*3600`)
4. Run the simulation
5. Results (bus voltage, PV/wind output, grid exchange, battery SOC, cumulative energy) will appear in Scope and Display blocks
...
- Improved PV and wind turbine models:
  Enhanced output curtailment based on baseline over-voltage.
  This significantly reduces voltage rise on the DC bus.
- Added new Simulink models: `Residential_grid8.slx`, `Residential_grid9.slx`
- Legacy models (`Residential_grid5.slx`, `Residential_grid7.slx`) are kept for reference.
- Updated user manuals (EN/JP) to reflect the new control behavior.
